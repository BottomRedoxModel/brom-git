...
!=======================================================================================================================
 ...
 !_______BIG Cycle ("i_day"=0,...,last_day-1)________!
    ...
    do i_day=0,(last_day-1)
            ...
            do i=i_min, i_water
                ...
                do ip=1,par_max !Sum over contributions from each particulate variable
                    if (is_solid(ip).eq.1) then
                        dVV(i,k,1)= dVV(i,k,1)+dcc(i,k,ip)/rho(ip)
                    endif
                enddo
            enddo
       ...
       if (id.eq.1) write (*,'(a, i4, a, i4, a, e)') " model year:", model_year, "; julianday:", julianday, "; dVV(k_bbl_sed):", dVV(1,k_bbl_sed,1)
        ...
        do i=i_min, i_water
            if(model_w_sed.ge.1) then
                ...
                sink, dcc, dcc_R, dVV,bctype_top, bctype_bottom, bc_top, &
        ...
        do i=i_min, i_water
           ...
           do ip=1,par_max !Sum over contributions from each particulate variable
              if (is_solid(ip).eq.1) then
               ...
               ! w_1m(k_bbl_sed+1,ip) = 0. !wbio(i,k_bbl_sed,ip)*min(cc(i,k_bbl_sed,ip),cc(i,k_bbl_sed-1,ip))
                !Now set rhop_i*(1-phi)*w_1i in the sediments by integrating the reaction terms
                dVV(i,k,1)= dVV(i,k,1)+dcc_R(i,k,ip)/rho(ip)
                !do k=k_bbl_sed+2,k_max+1
                !    w_1m(k,ip) = w_1m(k-1,ip) + 0.0! + dcc_R(i,k-1,ip)*hz(k-1)
                !end do
                !w_1m(k_sed1,ip) = w_1m(k_sed1,ip) !/ (rho(ip)*(1.0_rk-phi_fresh)) !Divide by rhop_i*(1-phi) to get w_1c(i)
