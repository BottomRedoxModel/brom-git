...
!=======================================================================================================================
    subroutine calculate_sed_eya(i, k_max, par_max, model, cc, wti, sink, &
        dcc, dcc_R, dVV, bctype_top, bctype_bottom, bc_top, bc_bottom, &
...
!        i = real(i)
    ...
    ! we accelerate burying rate due to an increase of particles volume dVV()[m3/sec] in water layer just above SWI
        do k=k_bbl_sed,k_max-1
            do ip=1,par_max
                wti(i,k,ip) = wti(i,k,ip) + max(0.0_rk,dVV(i,k_bbl_sed,1))/fresh_PM_poros/dz(k)
            end do
        enddo
    endif
    ...
    do idf=1,freq_sed
        ...
        do ip=1,par_max
            do k=1,k_max
                sink(i,k,:) = ((100000.0_rk*wti(i,k,:))*cc(i,k,:))/100000.0_rk
            end do
        enddo
